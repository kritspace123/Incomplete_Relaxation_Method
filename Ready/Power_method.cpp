...
public:
    ...
    void Print(){
        ...
        cout << "Собственное число:" << lamda << endl;
...
double Power_Method(double** m_Arr, int N, double* m_Y, eigenvector** vectors, int id){
    int counter = 0;
...
double Power_Method_for_minim(double** m_Arr, int N, double* m_Y, double a, eigenvector** vectors){
...
void Power_method_for_all_numbers(double** m_Arr, int N, double* Y, eigenvector** vectors){

    cout << "Наибольшее с.ч и с.в:" << endl;
    for (int i = 0; i < N; ++i) {
        Power_Method(m_Arr, N, Y, vectors, i);
        vectors[i]->Print();
...
void maxim_minim(double** m_Arr, int N, double* Y,  eigenvector** vectors){
    Power_Method(m_Arr, N, Y, vectors, 0);
    cout << "Наибольшее с.ч и с.в:" << endl;
    vectors[0]->Print();
    double a = vectors[0]->GetLamda();
    Power_Method_for_minim(m_Arr, N, Y,a, vectors);
    cout << "Наименьшее с.ч и с.в:" << endl;
    vectors[1]->Print();
}
...
int main(){
    system("chcp 65001");
    cout <<endl << endl;
    ifstream FI("input777.txt"); //открываем файл "input777.txt"
    if(!FI){
    ...
    Power_method_for_all_numbers(m_Arr, N, m_Y, Sov);
//    maxim_minim(m_Arr, N, m_Y, Sov);
